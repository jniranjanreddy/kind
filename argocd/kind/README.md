...
- Sync policy: automated, with `selfHeal` and `prune` — any push to the
  `kind` branch auto-syncs to the cluster, manual `kubectl` drift gets
...
`kind-*`, to avoid installing kind-sized config against the wrong cluster.
...
3. Applies [application.yaml](application.yaml), registering the `app`
   Application against the `kind` branch.
...
**Note:** as of this writing the `kind` branch only has a `README.md` — the
`k8s` path this Application points to doesn't exist yet. Argo CD will report
the Application as an error until you add manifests under `k8s/` on that
branch and push. That's expected, not a bug in this script.
