...
# of a local kind cluster, using the postgres-values.yaml and
...
case "$CONTEXT" in
  kind-*) ;;
  *)
    echo "ERROR: current kubectl context '$CONTEXT' doesn't look like a kind cluster (expected 'kind-*'). Switch context, or use temporal.sh / minikube/temporal.sh for other clusters." >&2
    exit 1
    ;;
esac
...
# kind's single node has no such labels by default, so label it here.
